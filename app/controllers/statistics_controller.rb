class StatisticsController < ApplicationController
  ...
  def create
	@user_critic_hivecoin = (Statistic.where(:user_id => params[:statistic][:user_id]).count+1) % 3
	@user = User.find(params[:statistic][:user_id])
    @statistic = Statistic.new(params[:statistic])
    respond_to do |format|
      if @statistic.save
		if @user_critic_hivecoin == 0
			@user.update_attributes(:total_hivecoins => @user.total_hivecoins + 1)
		end 
        format.html { redirect_to request.referer, notice: 'Statistic was successfully created.' }
